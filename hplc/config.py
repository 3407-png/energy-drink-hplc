...
# ---------------------------------------------------------------------------
# 4-2. 외부 검량선을 CSV 대신 계수로 직접 넣기
# ---------------------------------------------------------------------------
#
# 변환 상수는 '외부 검량선으로 읽은 겉보기 농도'와 비교하는 값이라 검량선이
# 있어야 계산된다. 보통은 CSV에 group=calib 행을 넣으면 되지만, 검량선을 이미
# 다른 곳에서 구해 두었고 계수만 알고 있다면 여기에 적어도 된다.
#
#   EXTERNAL_CALIBRATION = {
#       "caffeine": (10267.4, 0.0),        # (기울기, y절편) — 면적/ppm, 바이알 농도 기준
#       "sodium_benzoate": (45593.2, 0.0),
#   }
#
# 주의: 여기에 적은 계수는 직선성(R2)이나 LOD/LOQ 정보를 담지 않으므로,
# 보고서의 해당 칸은 '-' 로 나온다. 가능하면 CSV에 검량선 행을 넣는 편이 낫다.
EXTERNAL_CALIBRATION: dict[str, tuple[float, float]] = {}
